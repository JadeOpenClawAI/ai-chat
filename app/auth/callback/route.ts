...
export async function GET(req: NextRequest) {
  ...
  if (error) {
    return redirectWithCookieClear(`${settingsUrl}?oauth_error=${encodeURIComponent(errorDescription ?? error)}`, state);
  }
  if (!code || !state) {
    return redirectWithCookieClear(`${settingsUrl}?oauth_error=missing_params`, state);
  }
  ...
  if (!codeVerifier) {
    return redirectWithCookieClear(`${settingsUrl}?oauth_error=invalid_state`, state);
  }
