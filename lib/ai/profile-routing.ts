...
export function parseInChatCommand(input: string):
  ...
  if (profileMatch) {
    return { type: 'profile', profileId: profileMatch[1]! };
  }

  const modelMatch = value.match(/^\/model\s+([^\s]+)$/);
  if (modelMatch) {
    return { type: 'model', modelId: modelMatch[1]! };
  }

  const routeMatch = value.match(/^\/route\s+primary\s+([^\s]+)\s+([^\s]+)$/);
  if (routeMatch) {
    return { type: 'route-primary', profileId: routeMatch[1]!, modelId: routeMatch[2]! };
  }
