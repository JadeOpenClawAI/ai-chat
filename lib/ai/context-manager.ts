...
function trimSummaryCompactionToTarget(
  ...
  if (!hasSummary || getMessagesTokenCount(compacted, model) <= targetTokens) {
    return compacted;
  }
  ...
  for (let i = 0; i < 8 && body.length > 240; i += 1) {
    ...
    if (getMessagesTokenCount(candidate, model) >= getMessagesTokenCount(compacted, model)) {
      break;
    }
    compacted = candidate;
    if (getMessagesTokenCount(compacted, model) <= targetTokens) {
      break;
    }
  }
